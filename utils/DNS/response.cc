  ...
  while (label_length != 0) {
	...
	return -1;
	 ...
	 of bytes read by 2 (for the pointer value). */
      read_domain_name((char*)new_packet_index, packet_start, 
...
struct dns_response *parse_dns_response(void *packet_buffer, 
  ...
  *answer_count = 0;
  ...
  bytes_read = read_domain_name((char*)buffer_index, (char*)packet_buffer, 
  ...
  *answer_count = header.an_count;
  ...
  for (i = 0; i < header.an_count; ++i) {
    ...
    bytes_read = read_domain_name((char*)buffer_index, (char*)packet_buffer, packet_length, buffer);
    if (bytes_read == -1) {
      free(responses);
      strncpy(error_message, "response has invalid format", ERROR_BUFFER);
      return 0;
    ...
    case DNS_NS_RECORD:
      bytes_read = read_domain_name((char*)buffer_index, (char*)packet_buffer, packet_length, responses[i].name);
      if (bytes_read == -1) {
	free(responses);
	strncpy(error_message, "response has invalid format", ERROR_BUFFER);
	return 0;
